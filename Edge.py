...
class Crawler(object):
    ...
    def __init__(self, proxy=None):
        ...
        if proxy:
            ...
            edge_options.add_argument('--proxy-server=%s' % proxy_address)
            logging.info('Edge using proxy: %s', proxy['https'])

        self.edge = webdriver.Edge(options=edge_options) #, desired_capabilities=caps
        # jd sometimes load google pic takes much time
        self.edge.set_page_load_timeout(30)
        # set timeout for script
        self.edge.set_script_timeout(30)
    ...
    def get_jd_item(self, url):
        ...
        try:
            ...
            while retry:
                try:
                    element = self.edge.find_element("xpath","//*[@class='p-price']/span[2]").text
                    if element:
                        logging.info("爬取价格数据")
        ...
        # 提取商品名称
        try:
            name = self.edge.find_element("xpath","//*[@class='sku-name']").text
            item_info_dict['name'] = name
        except AttributeError as e:
            logging.warning('Crawl name failure: {}'.format(e))
        except NoSuchElementException:
            try:
                name = self.edge.find_element("xpath","//*[@class='name']").text
                item_info_dict['name'] = name
        ...
        try:
            # global found
            if self.found:
                price = self.edge.find_element("xpath","//*[@class='p-price']").text
                if price:
